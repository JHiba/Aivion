# bg_removal.py (with positioning + positioning preview + click-to-place FIXED)
import gradio as gr
...
# =========================
# NEW helpers for positioning & scaling
# =========================
def _scale_rgba(img_rgba: Image.Image, scale_pct: int) -> Image.Image:
    """Scale an RGBA image by percentage (e.g., 30..200)."""
...
def _anchor_base_xy(bg_w: int, bg_h: int, fg_w: int, fg_h: int, anchor: str) -> Tuple[int, int]:
    """Return top-left (x,y) for a given anchor on the background canvas."""
    anchors = {
        "Top-Left":        (0, 0),
        "Top":             ((bg_w - fg_w)//2, 0),
...
# =========================
# Replacement core (with positioning)
# =========================
...
def replace_bg_step(
    ...
    std_on, std_size, std_mode, std_transparent, std_pad_color,
    scale_pct=100, offset_x=0, offset_y=0, anchor="Center", clamp_inside=True
):
    """
    Replacement with positioning and scaling.

    - Composes on a canvas:
        * If std_on=True -> square canvas (std_size x std_size)
        * Else            -> canvas size = removed image size
    - Places the foreground by anchor + (offset_x, offset_y) after scaling.
    """
    if removed_img is None:
        return None, "No removed image available. Run Remove Background first."

    # Ensure FG has alpha and scale it
    fg, flag = ensure_alpha(removed_img.convert("RGBA"))
    fg = _scale_rgba(fg, int(scale_pct))
    fg_w, fg_h = fg.size

    # Determine canvas size for composition
    if std_on:
        canvas_w = canvas_h = int(std_size)
    else:
        canvas_w, canvas_h = removed_img.size

    # Build/prepare background
    ...
    # Compute placement from anchor + offsets
    ...
    if std_on:
        debug += f" | standardized canvas: True -> {canvas_w}px {std_mode} | placed at: ({x},{y}), scale: {scale_pct}%"
...
def remove_bg_batch(
    ...
    input_files = input_files[:4]  # keep first 4, as intended
    ...
    # If nothing processed, don't return an empty zip path
...
# =========================
# Click-to-place helpers (preview only)
# =========================
def _pos_click_coords(evt: gr.SelectData):
    """
    Gradio passes click data via gr.SelectData.
    For an Image, evt.index is (x, y) in image pixel coords.
    """
...
def preview_position_from_click(
    removed_img, replace_option, color_choice, bg_upload,
    std_on, std_size, std_mode, std_transparent, std_pad_color,
    scale_pct, click_x, click_y, clamp_inside=True
):
    """
    Convert a click (center placement) to offsets and reuse replace_bg_step.
    """
    if removed_img is None or click_x is None or click_y is None:
        return None, "Click on the preview to place the product."

    # Scale the FG to know its size, compute canvas size (same logic as replace_bg_step)
    ...
    # Base placement for Center anchor (top-left that would center the FG)
    base_x = (canvas_w - fg_w) // 2
    base_y = (canvas_h - fg_h) // 2

    # Desired top-left if the user clicked where the FG center should go
    desired_x = int(click_x) - fg_w // 2
    desired_y = int(click_y) - fg_h // 2

    # Convert to offsets relative to Center anchor
    offset_x = desired_x - base_x
    offset_y = desired_y - base_y

    # Reuse compositor
    return replace_bg_step(
        removed_img, replace_option, color_choice, bg_upload,
        std_on, std_size, std_mode, std_transparent, std_pad_color,
        scale_pct=scale_pct, offset_x=offset_x, offset_y=offset_y,
        anchor="Center", clamp_inside=clamp_inside
    )
...
with gr.Blocks(css=css, title="Aivion — Enhanced Background Removal") as demo:
    gr.Markdown("#  Aivion — Background Removal")
    gr.Markdown("Tab 1: **Removal & Replacement** · Tab 2: **Enhancements (Preview & Apply)** · Plus Model Comparison & About.")
    ...
    with gr.Tab(" Background Removal & Replacement"):
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                # Expanded range ±1200 as requested
                ...
                # --- Positioning preview ---
                gr.Markdown("##### Positioning Preview")
                pos_preview = gr.Image(type="pil", label="Preview (position only)", elem_classes="gr-image")
                pos_dbg = gr.Textbox(label="Preview debug", interactive=False)
                btn_pos_preview = gr.Button("🔍 Preview Positioning", variant="secondary")

                # States to hold last click
                click_x_state = gr.State(None)
                click_y_state = gr.State(None)
        ...
        # Helper function that prioritizes click coords over sliders
        def preview_with_click_priority(
            removed_img, replace_option, color_choice, bg_upload,
            std_on, std_size, std_mode, std_transparent, std_pad_color,
            scale_pct, offset_x, offset_y, anchor, clamp_inside,
            click_x, click_y
        ):
            # If user clicked, use those coords; otherwise use slider values
            if click_x is not None and click_y is not None:
                return preview_position_from_click(
                    removed_img, replace_option, color_choice, bg_upload,
                    std_on, std_size, std_mode, std_transparent, std_pad_color,
                    scale_pct, click_x, click_y, clamp_inside
                )
            else:
                return replace_bg_step(
                    removed_img, replace_option, color_choice, bg_upload,
        ...
        # Positioning preview (button)
        btn_pos_preview.click(
            fn=preview_with_click_priority,
            inputs=[
                out_enh, rep_sel, rep_color, rep_bg,
                std_on, std_size, std_mode, std_transparent, std_pad_color,
                scale_pct_ui, offset_x_ui, offset_y_ui, anchor_ui, clamp_ui,
                click_x_state, click_y_state
            ],
            outputs=[pos_preview, pos_dbg]
        )

        # Click-to-place: capture click then re-render preview
        pos_preview.select(
            fn=_pos_click_coords,
            inputs=None,
            outputs=[click_x_state, click_y_state]
        ).then(
            fn=preview_position_from_click,
            inputs=[
                out_enh, rep_sel, rep_color, rep_bg,
                std_on, std_size, std_mode, std_transparent, std_pad_color,
                scale_pct_ui, click_x_state, click_y_state, clamp_ui
            ],
            outputs=[pos_preview, pos_dbg]
        )

        # Apply replacement (writes to final result)
        btn_replace.click(
            fn=preview_with_click_priority,
            inputs=[
                out_enh, rep_sel, rep_color, rep_bg,
        ...
        # Batch removal (now returns gallery, zip, status string)
        btn_batch.click(
            fn=remove_bg_batch,
    ...
    # ----------------------- TAB 2: Enhancements (Preview & Apply) -------------
    with gr.Tab(" Enhancements (Preview Only)"):
        with gr.Row():
            ...
            with gr.Column(scale=1):
                prev_img = gr.Image(type="pil", label="Enhanced Previeaw", elem_classes="gr-image")
...
##  Settings:
-  **Edge Smoothing**: 1.0–2.0 for softer edges  
-  **No Smoothing**: Keep 0 for sharp cutouts  

**Workflow**
1. Tab 1: Remove background (single or batch) → preview positioning (click or sliders) → apply replacement → download.  
2. Tab 2: Tune enhancements → preview → remove with settings → download.
3. side note: We prefer you remove backgrounds separately for each product for efficient and smoother process.
        """)
